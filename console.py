...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        if not args:
            ...
            return
        class_name_text = args.strip().split(maxsplit=1)[0]
        args_list = args.strip().split()[1:]
        if class_name_text not in HBNBCommand.classes:
            print("** class doesn't exist **")
            return
        item_class = HBNBCommand.classes[class_name_text]
        # get kwargs
        kwargs = {}
        for arg in args_list:
            if "=" not in arg:
                ...
                key = arg.split('=', maxsplit=1)[0]
                val = arg.split('=', maxsplit=1)[1]
                if val[0] == '"' and val[-1] == '"':
                    left_trim = shlex.split(val)[0]
                    val = left_trim.replace('_', ' ')
                else:
                    try:
                        val = int(val)
                    except ValueError:
